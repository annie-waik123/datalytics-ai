  ...
  async function renderWidget(widgetId, stateSnapshot = builderStateRef.current) {
    ...
    try {
      const response = await renderDashboardWidget({
        ...
        mapping: widget.mapping,
// Settings are applied client-side so they are not sent to backend
// to avoid breaking the backend render cache.
  ...
  function handleRemoveField(slotKey, value) {
    if (!selectedWidget) return
    updateSelectedWidget((widget) => {
      const current = widget.mapping?.[slotKey]
      // Single-field slots store a string, not an array — just clear it
      if (!Array.isArray(current)) {
        return {
          ...widget,
          mapping: { ...widget.mapping, [slotKey]: null },
        }
      }
      // Multi-field slots store an array — filter out the removed value
      return {
        ...widget,
        mapping: {
          ...widget.mapping,
          [slotKey]: current.filter((item) => item !== value),
        },
      }
    })
    scheduleRender(selectedWidget.id)
  ...
  async function rerenderAll(stateSnapshot = builderStateRef.current) {
    // Process in smaller batches (e.g. 3 at a time) to prevent hammering the Python backend
    // which caused huge delays ("dynamic data change ... bahut time lag rha hai")
    const queue = [...stateSnapshot.widgets]
    while (queue.length > 0) {
      const batch = queue.splice(0, 3)
      await Promise.allSettled(batch.map((widget) => renderWidget(widget.id, stateSnapshot)))
    }
  }
  ...
  async function handleRefreshWidget(widgetId) {
    const widget = builderStateRef.current.widgets.find((item) => item.id === widgetId)
    if (widget) {
      const cacheKey = widgetRenderKey(widget, builderStateRef.current)
      ...
      <div className="powerbi-stats-grid">
        ...
        <div className="powerbi-stat-actions">
          ...
          <button 
            ...
            disabled={!builderState.crossFilter?.column}
            style={{ opacity: !builderState.crossFilter?.column ? 0.4 : 1, cursor: !builderState.crossFilter?.column ? 'not-allowed' : 'pointer' }}
          >
            Clear Cross Filter
          </button>
          ...
          <button 
            ...
            disabled={!builderState.globalFilters?.length}
            style={{ opacity: !builderState.globalFilters?.length ? 0.4 : 1, cursor: !builderState.globalFilters?.length ? 'not-allowed' : 'pointer' }}
          >
