...
// Safely coerce multi-field slot values to arrays (handles stale string values in saved state)
function normalizeMapping(raw = {}) {
  const MULTI_SLOTS = ['values', 'secondary_values', 'tooltip', 'rows', 'columns']
  const out = { ...emptyWidgetMapping(), ...raw }
  MULTI_SLOTS.forEach((key) => {
    const v = out[key]
    if (!Array.isArray(v)) {
      out[key] = v ? [v] : []
    }
...
export function createWidget(chartType, overrides = {}) {
  ...
  return {
    ...
    mapping: normalizeMapping(overrides.mapping),
    layout: {
...
export function selectedColumnsFromMapping(mapping = {}) {
  // Safely coerce a value to an array — never spread a string character-by-character
  ...
  const values = [
    ...
    mapping.longitude,
    mapping.target,
    ...toArr(mapping.secondary_values),
    ...toArr(mapping.values),
    ...toArr(mapping.tooltip),
    ...toArr(mapping.rows),
    ...toArr(mapping.columns),
  ].filter(Boolean)
